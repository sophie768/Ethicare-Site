...
(() => {
  ...
  const stylesheet = `
    ...
    /* Full-bleed cover: renders before the paginated table, so it escapes the
     * header/footer spacers and fills the whole first page, all four edges. */
    ::slotted([slot="cover"]) {
      display: block; box-sizing: border-box; overflow: hidden;
      width: var(--doc-page-w); height: var(--doc-page-h);
      margin: calc(-1 * var(--doc-page-margin)) calc(-1 * var(--doc-page-margin)) 0;
      break-after: page;
    }
    @media print {
      ...
      ::slotted([slot="cover"]) {
        width: auto; height: var(--doc-page-h);
        margin: 0 calc(-1 * var(--doc-page-margin));
      }
